...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      localStorage.setItem("x402_session", JSON.stringify({ email, loggedIn: true }));
      router.push("/");
    }, 800);
  };
    ...
    <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-4 py-16">
      <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 p-8">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-zinc-400">Sign in to your audit account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link href="/2fa/verify" className="text-sm font-medium text-indigo-400 hover:text-indigo-300">
              Forgot password?
            </Link>
          </div>
        ...
        <button className="flex w-full items-center justify-center gap-3 rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-zinc-700">
